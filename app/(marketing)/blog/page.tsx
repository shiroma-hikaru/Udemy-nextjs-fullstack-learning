            ...
            <div className="grid sm:grid-cols-2 gap-10">
                {posts.map((post)=> (
                    <article key={post._id} className="relative flex flex-col space-y-2">
                        {post.image && (
                            <Image className="rounded-md border bg-muted" src={post.image} alt={post.title} width={804} height={452} />
                        )}
                        ...
                        {post.date && (
                            <p className="text-sm text-muted-foreground">{format(new Date(post.date), "yyyy/MM/dd")}</p>
                        )}
